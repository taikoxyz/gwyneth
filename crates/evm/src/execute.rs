...
use revm_primitives::{db::SyncDatabase, U256};
...
pub trait BlockExecutionStrategyFactory: Send + Sync + Clone + Unpin + 'static {
    ...
    type Strategy<DB: SyncDatabase<Error: Into<ProviderError> + Display>>: BlockExecutionStrategy<
        DB,
        Error = BlockExecutionError,
    >;
    ...
    DB: SyncDatabase<Error: Into<ProviderError> + Display>,
{
    ...
    fn finalize(mut self) -> Self::Output {
        println!("using incorrect chain id!");
