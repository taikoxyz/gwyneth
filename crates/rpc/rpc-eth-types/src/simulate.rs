...
use reth_revm::database::SyncStateProviderDatabase;
use reth_rpc_server_types::result::rpc_err;
use reth_rpc_types_compat::{block::from_block, TransactionCompat};
use reth_storage_api::StateRootProvider;
use reth_trie::{HashedPostState, HashedStorage};
use revm::{db::CacheDB, SyncDatabase};
use revm_primitives::{keccak256, Address, BlockEnv, Bytes, ChainAddress, ExecutionResult, TxKind, B256, U256};
...
/// built right away.
pub fn resolve_transactions<DB: SyncDatabase>(
    ...
    EthApiError: From<DB::Error>,
{
    ...
    for tx in txs {
        ...
        if tx.nonce.is_none() {
            tx.nonce = Some(db.basic(ChainAddress(chain_id, from))?.map(|acc| acc.nonce).unwrap_or_default());
        }
...
pub fn build_block<T: TransactionCompat<Error: FromEthApiError>>(
    results: Vec<(ChainAddress, ExecutionResult)>,
    transactions: Vec<TransactionSigned>,
    block_env: &BlockEnv,
    parent_hash: B256,
    total_difficulty: U256,
    full_transactions: bool,
    db: &CacheDB<SyncStateProviderDatabase<StateProviderTraitObjWrapper<'_>>>,
    tx_resp_builder: &T,
    ...
    // TODO(merge)
    let state_root = db.db.get_db(1).unwrap().state_root(hashed_state).map_err(T::Error::from_eth_err)?;
    ...
    let block = BlockWithSenders {
        ...
        senders: senders.into_iter().map(|a| a.1).collect(),
    };
