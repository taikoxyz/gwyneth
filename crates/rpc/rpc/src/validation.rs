...
use reth_revm::{cached::CachedReads, database::{SyncStateProviderDatabase, StateProviderDatabase}};
use reth_rpc_api::BlockSubmissionValidationApiServer;
use reth_rpc_eth_types::EthApiError;
use reth_rpc_server_types::{result::internal_rpc_err, ToRpcResult};
use reth_trie::HashedPostState;
use revm_primitives::{Address, ChainAddress, B256, U256};
    ...
    E: BlockExecutorProvider,
{
    ...
    pub async fn validate_message_against_block(
        ...
        //let cached_db = request_cache.as_db_mut(SyncStateProviderDatabase::new(None, StateProviderDatabase::new(&state_provider)));
        //let executor = self.executor_provider.executor(reth_revm::cached::to_sync_cached_reads(cached_db, 1)); // TODO(merge)
        let cached_db = SyncStateProviderDatabase::new(None, StateProviderDatabase::new(&state_provider));
        let executor = self.executor_provider.executor(cached_db); // TODO(merge)
    ...
    fn ensure_payment(
        ...
        let (mut balance_before, balance_after) = if let Some(acc) =
            output.state.state.get(&ChainAddress(1, message.proposer_fee_recipient)) // TODO(merge)
        {
