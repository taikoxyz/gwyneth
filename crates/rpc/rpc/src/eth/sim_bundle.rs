...
use reth_revm::database::{SyncStateProviderDatabase, StateProviderDatabase};
...
use revm::{
    db::CacheDB,
    primitives::{Address, ChainAddress, EnvWithHandlerCfg, ResultAndState, SpecId, TxEnv},
    DatabaseCommit, DatabaseRef, SyncDatabaseRef,
};
    ...
    Eth: EthTransactions + LoadPendingBlock + Call + 'static,
{
    ...
    async fn sim_bundle(
        ...
        if let Some(coinbase) = coinbase {
            block_env.coinbase = ChainAddress(1, coinbase);
        }
        ...
        let sim_response = self
            ...
            .spawn_with_state_at_block(current_block, move |state| {
                ...
                let db = CacheDB::new(SyncStateProviderDatabase::new(None, StateProviderDatabase::new(state)));

                let initial_coinbase_balance = SyncDatabaseRef::basic_ref(&db, coinbase)
