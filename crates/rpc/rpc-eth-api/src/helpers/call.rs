...
use revm::{Database, DatabaseCommit, GetInspector, SyncDatabase, SyncDatabaseRef};
            ...
            self.spawn_with_state_at_block(block, move |state| {
                ...
                for block in block_state_calls {
                    ...
                    if let Some(block_overrides) = block_overrides {
                        apply_block_overrides(cfg.chain_id, block_overrides, &mut db, &mut block_env);
                    }
...
pub trait Call: LoadState<Evm: ConfigureEvm<Header = Header>> + SpawnBlocking {
        ...
        EthApiError: From<<DB as SyncDatabaseRef>::Error>,
    {
        ...
        if let Some(block_overrides) = overrides.block {
            apply_block_overrides(cfg.chain_id, *block_overrides, db, &mut block);
        }
        if let Some(state_overrides) = overrides.state {
            apply_state_overrides(cfg.chain_id, state_overrides, db)?;
        }
