...
use revm::{db::CacheDB, Database, DatabaseCommit, GetInspector, Inspector, SyncDatabase};
...
pub trait Trace: LoadState<Evm: ConfigureEvm<Header = Header>> {
    ...
    /// already applied.
    fn apply_pre_execution_changes<DB: Send + SyncDatabase<Error: Display> + DatabaseCommit>(
        &self,
