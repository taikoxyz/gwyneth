...
use revm_primitives::{
    calc_excess_blob_gas, BlockEnv, CfgEnvWithHandlerCfg, ChainAddress, EVMError, EnvWithHandlerCfg,
};
use std::{
    ...
    Spec: EthereumHardforks,
{
    ...
    let mut state = State::builder()
        .with_database_ref(SyncStateProviderDatabase::new(
            //Some(chain_spec.chain().id()), // TODO(merge)
            None,
    ...
    if let Some(withdrawals) = &reorg_target.body.withdrawals {
        state.increment_balances(post_block_withdrawals_balance_increments(
            ...
            reorg_target.timestamp,
            withdrawals,
        ).iter().map(|(a, v)| (ChainAddress(1 /* Brecht(merge) */, *a), *v)))?;
    }
