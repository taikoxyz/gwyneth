...
use reth_revm::{
    database::{SyncStateProviderDatabase, StateProviderDatabase},
    db::states::bundle_state::BundleRetention,
    primitives::{BlockEnv, CfgEnvWithHandlerCfg, ChainAddress, EnvWithHandlerCfg},
    ...
    EvmConfig: ConfigureEvm<Header = Header>,
{
    fn on_invalid_block(
        ...
        let mut db = StateBuilder::new()
            .with_database(SyncStateProviderDatabase::new(None, StateProviderDatabase::new( // TODO(merge)
        ...
        for (address, account) in db.cache.accounts {
            ...
            if let Some(account) = account.account {
                state_preimages.insert(hashed_address, alloy_rlp::encode(address.1).into());
        ...
        let state = state_provider.get(&chain_id).unwrap().witness(Default::default(), hashed_state.clone())?;
        ...
        let (re_executed_root, trie_output) =
            state_provider.get(&chain_id).unwrap().state_root_with_updates(hashed_state)?;
